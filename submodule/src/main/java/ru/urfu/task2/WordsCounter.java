...
public class WordsCounter
{
     ...
     *
     * Сложность O(n + 16*n) = O(17*n). n - считывание всех символов, преобразовние их в слова и подсчет количества раз, сколько слово встретилось
     * Подсчет количества делается за O(1) - т.к. работаем с HashSet, где основные операции O(1).
     * После итерируемся по всем словам и засовываем их в очередь с приоритетом для наиболее и наименее встречаюшихся слов = O (n * ( 2 * (4 + 4) ) )=
     * O( 16(n) )
     * Добавление в очередь с приоритетом(O(4 + 4)): добавление в очередь с приоритетом работает за log(n), удаление тоже
     * После добавление элемента, мы проверяем их количество, если оно больше 10, тогда мы удаляем элемент с начала,
     * из этого следует, что в очереди всегда будет <= 11 элементов, соответственно добавление O(log(11)) = 4, удаление (log(11)) = 4
     */
